...
kp920CmpReproduce() {
  ...
  for config in "${cmpConfigs[@]}"; do
    ...
    for key in "${!dataset[@]}"; do
      cycle=${dataset[$key]}
      timeElapsed=$(python3 -c "print($cycle/($freq*1000000))")
...
scalabilityReproduce() {
  ...
  for config in "${cmpConfigs[@]}"; do
    for dramEnable in 0 1; do
      DRAMSIM3_ENABLE="$dramEnable" ./run_emulator.sh "$config" "sgemm_64"
      cycle=$(readCycle)
...
x60CmpReproduce() {
  ...
  X60_COMPARE_DATA_TABLE="Config,Case Name,Cycle,Time Elapsed in Secs (1.6GHz)\n"
  for config in "${cmpConfigs[@]}"; do
    declare -A dataset=()
    ...
    for key in ${!dataset[@]}; do
      cycle=${dataset["$key"]}
      timeElapsed=$(python3 -c "print($cycle/($freq*1_000_000))")
